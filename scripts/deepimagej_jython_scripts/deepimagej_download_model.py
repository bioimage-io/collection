...
def convert_npy_to_tif(folder_path, test_name, axesOrder, name=CREATED_INPUT_SAMPLE_NAME):
...
parser.add_argument('-models_dir', type=str, required=True, help='Directory where models are going to be saved')
...
with open(macro_path, "a") as file:
    ...
    if " " in mfp:
        macro = MACRO_STR.format(model_path="[" + mfp + "]", input_path="[" + os.path.join(mfp, sample_name) + "]", output_folder="[" + os.path.join(mfp, "outputs") + "]")
    else:
        macro = MACRO_STR.format(model_path=mfp, input_path=os.path.join(mfp, sample_name), output_folder=os.path.join(mfp, "outputs"))
