    ...
    with open(os.path.join(model_dir, os.getenv("JSON_OUTS_FNAME")), 'r') as f:
        ...
        for i, output in enumerate(expected_outputs):
            ...
            if not os.path.exists(dij_output):
                ...
                raise Exception(f"Cannot find expected output {i}")
            ...
            assert dij_shape == expected_shape, f"Output {i} in deepiamgej has different shape {dij_shape} vs {expected_shape}"
            ...
            while dij_cursor.hasNext():
                ...
                if dij_cursor.get().getRealFloat() - expected_cursor.get().getRealFloat() > 1e-5:
                    raise Exception(f"Values of output {i} differ")
